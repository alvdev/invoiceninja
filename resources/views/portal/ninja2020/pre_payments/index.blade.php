...
@endpush
@section('body')
    <form action="{{ route('client.pre_payments.process') }}" method="post" id="payment-form"
                ...
                <div class="col-span-6 md:col-start-2 md:col-span-4">
                    ...
                    <div class="mb-4 overflow-hidden bg-white shadow sm:rounded-lg">
                        ...
                        @component('portal.ninja2020.components.general.card-element', ['title' => ctrans('texts.payment_details')])
                            ...
                            @if ($errors->has('notes'))
                                <p class="mt-2 text-red-900 border-red-300 px-2 py-1 bg-gray-100">{{ $errors->first('notes') }}
                                </p>
                            @endif
                        @endcomponent
                        ...
                        @component('portal.ninja2020.components.general.card-element', ['title' => $title])
                            <input type="text" class="input mt-0 mr-4 relative" name="amount" placeholder="" />
                            ...
                            @if ($errors->has('amount'))
                                <p class="mt-2 text-red-900 border-red-300 px-2 py-1 bg-gray-100">{{ $errors->first('amount') }}
                                </p>
                            @endif
                        @endcomponent
                        ...
                        @if ($allows_recurring)
                            <div x-data="{ show: false }">
                                @component('portal.ninja2020.components.general.card-element', ['title' => ctrans('texts.enable_recurring')])
                                    <input x-on:click="show = !show" id="is_recurring" aria-describedby="recurring-description"
                                ...
                                <div x-cloak x-show="show">
                                    @component('portal.ninja2020.components.general.card-element', ['title' => ctrans('texts.number_of_payments')])
                                        <select name="remaining_cycles" class="form-select input w-full bg-white">
                                            ...
                                            @for ($i = 1; $i < 60; $i++)
                                                <option value={{ $i }}
                                                    @if ($i == 1) selected @endif>{{ $i }}
                                                </option>
                                            @endfor
                                        </select>
                                        <span class="py-2">
                                            <label for="remaining_cycles"
                                                class="col-form-label text-center col-lg-3 text-gray-900">{{ ctrans('texts.number_of_payments_helper') }}</label>
                                        </span>
                                    @endcomponent
                        ...
                        <div class="px-4 py-3 bg-gray-50 text-right sm:px-6" x-data="{ buttonDisabled: false }">
                            <button
                                class="button button-primary bg-primary whitespace-nowrap"x-on:click="buttonDisabled = true"
                                x-bind:disabled="buttonDisabled">{{ ctrans('texts.pay_now') }}</button>
                        </div>
                    </div>
