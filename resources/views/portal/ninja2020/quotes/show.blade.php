...
@section('meta_title', ctrans('texts.entity_number_placeholder', ['entity' => ctrans('texts.quote'), 'entity_number' =>
    $quote->number]))

    @push('head')
        <meta name="show-quote-terms" content="{{ $settings->show_accept_quote_terms ? true : false }}">
        <meta name="require-quote-signature"
            content="{{ $client->company->account->hasFeature(\App\Models\Account::FEATURE_INVOICE_SETTINGS) && $settings->require_quote_signature }}">
            ...
            <div class="">
                <div class="sm:flex sm:items-start sm:justify-between">
                    <div>
                        ...
                        @if ($key)
                            <div class="btn hidden md:block" data-clipboard-text="{{ url("client/quote/{$key}") }}"
                                aria-label="Copied!">
                    ...
                    @if ($quote->invoice()->exists())
                        <div class="mt-5 sm:mt-0 sm:ml-6 flex justify-end">
                            <div class="inline-flex rounded-md shadow-sm">
                                <a class="button button-primary bg-primary whitespace-nowrap"
                                    href="/client/invoices/{{ $quote->invoice->hashed_id }}">{{ ctrans('texts.view_invoice') }}</a>
                            </div>
                        </div>
                    @endif
                </div>
...
@section('footer')
    ...
    @include('portal.ninja2020.invoices.includes.terms', [
        'entities' => [$quote],
        'entity_type' => ctrans('texts.quote'),
    ])
