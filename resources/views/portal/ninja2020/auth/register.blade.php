    ...
    <div class="grid lg:grid-cols-12 py-8">
        <div class="col-span-12 lg:col-span-8 lg:col-start-3 xl:col-span-6 xl:col-start-4 px-6">
            ...
            @elseif(isset($register_company) && !is_null($register_company))
                <div class="flex justify-center">
                    <img src="{{ $register_company->present()->logo() }}" class="mx-auto border-b border-gray-100 h-18 pb-4"
                        alt="{{ $register_company->present()->name() }} logo">
                </div>
            @endif
            ...
            <form id="register-form" action="{{ route('client.register', request()->route('company_key')) }}" method="POST"
                        ...
                        @foreach ($register_company->client_registration_fields as $field)
                            @if ($field['visible'])
                                <div class="col-span-12 md:col-span-6">
                                    ...
                                    @if ($field['key'] === 'email')
                                        <input id="{{ $field['key'] }}" class="input w-full" type="email"
                                            name="{{ $field['key'] }}" value="{{ old($field['key']) }}" />
                                    @elseif($field['key'] === 'password')
                                        <input id="{{ $field['key'] }}" class="input w-full" type="password"
                                            name="{{ $field['key'] }}" />
                                    @elseif($field['key'] === 'currency_id')
                                        <select id="currency_id" class="input w-full form-select bg-white"
                                            name="currency_id">
                                            @foreach (App\Utils\TranslationHelper::getCurrencies() as $currency)
                                                <option
                                                    {{ $currency->id == $register_company->settings->currency_id ? 'selected' : null }}
                                                    value="{{ $currency->id }}">
                                                    {{ $currency->name }}
                                                </option>
                                            @endforeach
                                    ...
                                    @elseif($field['key'] === 'country_id')
                                        <select id="shipping_country" class="input w-full form-select bg-white"
                                            name="country_id">
                                            <option value="none"></option>
                                            @foreach (App\Utils\TranslationHelper::getCountries() as $country)
                                                <option
                                                    {{ $country == isset(auth()->user()->client->shipping_country->id) ? 'selected' : null }}
                                                    value="{{ $country->id }}">
                                                    {{ $country->iso_3166_2 }}
                                                    ({{ $country->name }})
                                                </option>
                                    ...
                                    @else
                                        <input id="{{ $field['key'] }}" class="input w-full" name="{{ $field['key'] }}"
                                            value="{{ old($field['key']) }}" />
                                    @endif
                                ...
                                @if ($field['key'] === 'password')
                                    <div class="col-span-12 md:col-span-6">
                                        ...
                                        <input id="password_confirmation" type="password" class="input w-full"
                ...
                <div class="flex justify-between items-center mt-8">
                    ...
                    <a href="{{ route('client.login') }}"
                        class="button button-info bg-green-600 text-white">{{ ctrans('texts.login_label') }}</a>

                    <span class="inline-flex items-center" x-data="{ terms_of_service: false, privacy_policy: false }">
                        @if (
                            !empty($register_company->settings->client_portal_terms) ||
                                !empty($register_company->settings->client_portal_privacy_policy))
                        ...
                        @includeWhen(
                            !empty($register_company->settings->client_portal_terms),
                            'portal.ninja2020.auth.includes.register.popup',
                            [
                                'property' => 'terms_of_service',
                                'title' => ctrans('texts.terms_of_service'),
                                'content' => $register_company->settings->client_portal_terms,
                            ]
                        )
                        @includeWhen(
                            !empty($register_company->settings->client_portal_privacy_policy),
                            ...
                            [
                                ...
                                'title' => ctrans('texts.privacy_policy'),
                                'content' => $register_company->settings->client_portal_privacy_policy,
                            ]
                        )
                    ...
                    <button class="button button-primary bg-blue-600"
                        :disabled={{ $submitsForm == 'true' ? 'isSubmitted' : 'busy' }} x-on:click="busy = true">
