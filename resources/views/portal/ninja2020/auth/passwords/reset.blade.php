    ...
    <div class="grid lg:grid-cols-3">
        @if ($account && !$account->isPaid())
            <div class="hidden lg:block col-span-1 bg-red-100 h-screen">
                <img src="{{ asset('images/client-portal-new-image.jpg') }}" class="w-full h-screen object-cover"
        ...
        <div class="{{ $account && !$account->isPaid() ? 'col-span-2' : 'col-span-3' }} h-screen flex">
            <div class="m-auto w-1/2 md:w-1/3 lg:w-1/4">
                ...
                @elseif(isset($company) && !is_null($company))
                    <div>
                        <img src="{{ asset($company->present()->logo()) }}" class="h-14 mb-10"
                ...
                <div class="flex flex-col">
                    ...
                    <form action="{{ route('client.password.update') }}" method="post" class="mt-6">
                        ...
                        <div class="mt-5">
                            <button
                                class="button button-primary button-block bg-blue-600">{{ ctrans('texts.complete') }}</button>
                        </div>
                    </form>
                </div>
            </div>
