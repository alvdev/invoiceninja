...
@section('body')
    ...
    @if ($invoice->isPayable())
        <form
            action="{{ $settings->client_portal_allow_under_payment || $settings->client_portal_allow_over_payment ? route('client.invoices.bulk') : route('client.payments.process') }}"
            ...
            <div translate>
                <div class="sm:flex sm:items-center sm:justify-between">
                    <div>
                        <h3 class="text-2xl">
                            <span
                                class="font-semibold inline p-0 m-0">{{ ctrans('texts.invoice_number_placeholder', ['invoice' => $invoice->number]) }}
                            </span>
                            - {{ ctrans('texts.unpaid') }}
                        </h3>

                        @if ($key)
                            <div class="btn hidden md:block" data-clipboard-text="{{ url("client/invoice/{$key}") }}"
                                aria-label="Copied!">
                    ...
                    <div class="mt-5 sm:mt-0 sm:ml-6 flex justify-end">
                        <div>
                            ...
                            @if ($settings->client_portal_allow_under_payment || $settings->client_portal_allow_over_payment)
                                <button
                                    class="button button-primary bg-primary whitespace-nowrap">{{ ctrans('texts.pay_now') }}</button>
                            @else
        ...
        <div class="">
            <div class="">
                <div class="sm:flex sm:items-start sm:justify-between">
                    <div>
                        <h3 class="text-2xl">
                            <span class="font-semibold inline p-0 m-0">{{ ctrans('texts.invoice_number_placeholder', ['invoice' => $invoice->number]) }}</span>
