<div class="container mx-auto grid grid-cols-12 mb-4" data-ref="required-fields-container">
    <div class="col-span-12 lg:col-span-6 lg:col-start-4 overflow-hidden bg-white shadow rounded-lg">
        ...
        <form id="required-client-info-form"
            wire:submit.prevent="handleSubmit(Object.fromEntries(new FormData(document.getElementById('required-client-info-form'))))">
                    ...
                    @component('portal.ninja2020.components.general.card-element', ['title' => $field['label']])
                        @if ($field['name'] == 'client_country_id' || $field['name'] == 'client_shipping_country_id')
                            <select id="client_country" class="input w-full form-select bg-white" name="{{ $field['name'] }}"
                                wire:model.defer="{{ str_replace(['client_', '_line_', 'contact_'], ['client.', '', 'contact.'], $field['name']) }}">
                        ...
                        @if (session()->has('validation_errors') && array_key_exists($field['name'], session('validation_errors')))
                            <p class="mt-2 text-gray-900 border-red-300 px-2 py-1 bg-gray-100">
                                {{ session('validation_errors')[$field['name']][0] }}</p>
                        @endif
                    @endcomponent
                @endif
            @endforeach
            ...
            @if ($show_terms)
                ...
                @component('portal.ninja2020.components.general.card-element', ['title' => ctrans('texts.terms_of_service')])
                    <div x-data="{ open: false }">
                        <input wire:click="toggleTermsAccepted()" id="terms" name="terms_accepted" type="checkbox"
                            class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                        <a href="#"
                            class="group relative inline-block ml-4 text-blue-500 hover:text-red-500 duration-300 no-underline"
                            @click="open = true">{{ ctrans('texts.agree_to_terms', ['terms' => ctrans('texts.terms')]) }}</a>
