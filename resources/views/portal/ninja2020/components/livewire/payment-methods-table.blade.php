<div>
    <div class="flex items-center justify-between">
        <div class="relative" x-data="{ open: false }" x-on:click.away="open = false">
            ...
            @if ($client->getCreditCardGateway() || $client->getBankTransferGateway())
                <button x-on:click="open = !open" class="button button-primary bg-primary whitespace-nowrap"
                    data-cy="add-payment-method">{{ ctrans('texts.add_payment_method') }}</button>
                <div x-show="open" x-transition:enter="transition ease-out duration-100"
                    ...
                    <div class="py-1 rounded-md bg-white ring-1 ring-black ring-opacity-5">
                        @if ($client->getCreditCardGateway())
                            <a data-cy="add-credit-card-link"
                                href="{{ route('client.payment_methods.create', ['method' => App\Models\GatewayType::CREDIT_CARD]) }}"
                                class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition ease-in-out duration-150">
                                {{ ctrans('texts.credit_card') }}
                            </a>
                        @endif
                        @if ($client->getBankTransferGateway())
                            <a data-cy="add-bank-account-link"
                                href="{{ route('client.payment_methods.create', ['method' => $client->getBankTransferMethodType()]) }}"
                                class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition ease-in-out duration-150">
                                {{ ctrans('texts.bank_account') }}
                            </a>
                        @endif
                        @if ($client->getBACSGateway())
                            <a data-cy="add-bacs-link"
                                href="{{ route('client.payment_methods.create', ['method' => App\Models\GatewayType::BACS]) }}"
                                class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition ease-in-out duration-150">
                                {{ ctrans('texts.bacs') }}
                            </a>
                        @endif
    ...
    <div class="-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8 mt-4">
        <div class="align-middle inline-block min-w-full overflow-hidden rounded">
            <table class="min-w-full mt-4 payment-methods-table">
                <thead>
                    <tr>
                        <th
