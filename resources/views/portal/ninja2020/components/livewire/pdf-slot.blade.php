<div>
    <div class="flex flex-row space-x-2 float-right mb-2" x-data>
        <button wire:loading.attr="disabled" wire:click="downloadPdf" class="text-purple-700 hover:text-black" type="button">
            <span class="mr-0">{{ ctrans('texts.download_pdf') }}</span>
            <div wire:loading wire:target="downloadPdf">
                <svg class="animate-spin h-5 w-5 text-blue" xmlns="http://www.w3.org/2000/svg" fill="none"
                    viewBox="0 0 24 24">
        ...
        @if ($settings->enable_e_invoice && $entity_type == 'invoice')
            <button wire:loading.attr="disabled" wire:click="downloadEInvoice"
                class="bg-primary text-white px-4 py-4 lg:px-2 lg:py-2 rounded" type="button">
    ...
    <div class="hidden lg:block">
        <div wire:init="getPdf()">
            ...
            @if ($pdf)
                ...
                <iframe id="pdf-iframe" src="/{{ $route_entity }}/showBlob/{{ $pdf }}"
                    class="h-screen w-full border-0 mt-4"></iframe>
            @endif
        </div>
    </div>
