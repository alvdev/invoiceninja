                    ...
                    <div class="mt-4">
                        @foreach ($subscription->service()->getPlans() as $_subscription)
                            <button class="mt-8 mr-2">
                                <a class="border mt-4 bg-white rounded py-2 px-4 hover:bg-gray-100 text-sm"
                                    href="{{ route('client.subscription.purchase', $_subscription->hashed_id) }}">{{ $_subscription->name }}</a>
                            </button>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="col-span-12 w-full xl:col-span-9">
                    ...
                    <form wire:submit.prevent="authenticate" class="mt-8">
                        ...
                        @if ($steps['existing_user'])
                            ...
                            @if ($steps['passwordless_login_sent'])
                                <span class="block mt-2 text-sm text-green-600">{!! ctrans('texts.sent') !!}</span>
                            @endif
                        @endif
                ...
                @if (!empty($subscription->promo_code) && !$subscription->trial_enabled)
                    ...
                    <form wire:submit.prevent="handleCoupon" class="flex items-center mt-4">
                        ...
                        <button
                            class="button button-primary bg-primary whitespace-nowrap">{{ ctrans('texts.apply') }}</button>
                    </form>
                @endif
