    ...
    @else
        @component('portal.ninja2020.components.general.card-element-single')
            ...
            <label for="accept-terms"
                class="cursor-pointer">{{ ctrans('texts.ach_authorization', ['company' => auth()->user()->company->present()->name,'email' => auth()->guard('contact')->user()->client->company->settings->email]) }}</label>
        @endcomponent
        ...
        <div class="px-4 py-5 sm:px-6 lg:grid lg:grid-cols-3 lg:gap-4 lg:flex lg:items-center">
            ...
            <dd class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
                <button type="button" class="button button-primary bg-primary whitespace-nowrap" id="new-bank"
                    type="button">
        ...
        function confirmPayment(stripe, clientSecret) {
            stripe.confirmUsBankAccountPayment(clientSecret)
                .then(({
                    ...
                    } else if (paymentIntent.next_action?.type === "verify_with_microdeposits" || paymentIntent
