...
@push('head')
    ...
    <meta name="require-purchase_order-signature"
        content="{{ $purchase_order->company->account->hasFeature(\App\Models\Account::FEATURE_INVOICE_SETTINGS) && property_exists($settings, 'require_purchase_order_signature') && $settings->require_purchase_order_signature }}">
...
@section('body')
    ...
    @if ($purchase_order->company->getSetting('vendor_portal_enable_uploads'))
        @component('portal.ninja2020.purchase_orders.includes.upload', ['purchase_order' => $purchase_order])
        @endcomponent
    @endif

    @if (in_array($purchase_order->status_id, [
            \App\Models\PurchaseOrder::STATUS_SENT,
            \App\Models\PurchaseOrder::STATUS_DRAFT,
        ]))
        <div class="mb-4">
            @include('portal.ninja2020.purchase_orders.includes.actions', [
                ...
                <div class="sm:flex sm:items-start sm:justify-between">
                    <div>
                        <h3 class="text-lg leading-6 font-medium text-gray-900">
                            <span class="font-semibold inline p-0 m-0">{{ ctrans('texts.purchase_order_number_placeholder', ['purchase_order' => $purchase_order->number]) }}</span>
                            - {{ \App\Models\PurchaseOrder::stringStatus($purchase_order->status_id) }}
                        </h3>

                        @if ($key)
                            <div class="btn hidden md:block" data-clipboard-text="{{ url("vendor/purchase_order/{$key}") }}"
                                aria-label="Copied!">
                                <div class="flex text-sm leading-6 font-medium text-gray-500">
                                    <p class="pr-10">{{ url("vendor/purchase_order/{$key}") }}</p>
                                    <p><img class="h-5 w-5" src="{{ asset('assets/clippy.svg') }}" alt="Copy to clipboard">
                                    </p>
                                </div>
                            </div>
                        @endif
...
@section('footer')
    @include('portal.ninja2020.invoices.includes.terms', [
        ...
        'entity_type' => ctrans('texts.purchase_order'),
    ])
