...
@section('body')
    ...
    <div class="flex flex-col justify-center items-center mt-10" id="container-fastlink">
            ...
            @elseif(isset($company) && !is_null($company))
                <div class="max-h-28">
                    <img src="{{ $company->present()->logo() }}" class="mx-auto border-b border-gray-100 h-18 pb-4"
                        style="max-width: 400px;" alt="{{ $company->present()->name() }} logo">
                </div>
            @endif
        </div>
        ...
        <div id="cta" class="mb-4" x-data="{ open: false }">
            ...
            <button @click="open = !open" x-show="!open" type="submit" class="button button-primary bg-blue-600 my-4"
                id="btn-fastlink">{{ ctrans('texts.add_bank_account') }}</button>
        ...
        <div id="completed" class="mb-4">
            <a class="button button-primary bg-blue-600 my-4" href="{{ config('ninja.app_url') }}">Return to admin
                portal.</a>
        </div>
